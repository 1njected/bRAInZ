...
COMPOSE_FILES = $(if $(filter true,$(USETAILSCALE)),\
	$(if $(filter true,$(USEMCP)),-f docker-compose-tailscale-mcp.yaml,-f docker-compose-tailscale.yaml),\
	$(if $(filter true,$(USEMCP)),-f docker-compose-mcp.yaml,))
