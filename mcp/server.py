...
def search(query: str, top_k: int = 10, category: str = "", tag: str = "") -> list[dict]:
    """Search the bRAInZ library for items relevant to a topic or question.

    Use this as your primary tool for most requests. Call search to find relevant
    items, then call get_item on the results to read the full content, and
    synthesize the answer yourself from that material.
...
def query(question: str, category: str = "", top_k: int = 8) -> dict:
    """Ask bRAInZ to answer a question using its own RAG pipeline.

    Only use this when the user explicitly asks bRAInZ to answer something directly,
    or when you need a quick summarized answer and do not intend to reason over the
    source material yourself. For most tasks, prefer search + get_item so you can
    read and synthesize the content directly.

    Returns the answer text generated by bRAInZ and the source items it used.
