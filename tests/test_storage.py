...
class TestItemIndex:
    ...
    async def test_add_and_get(self, data_dir):
        ...
        idx = ItemIndex(data_dir)
        await idx.load()
        await idx.add("abc123", {**SAMPLE_ITEM, "content_hash": "sha256:aaa", "word_count": 10, "added": "2026-01-01T00:00:00Z", "updated": "2026-01-01T00:00:00Z", "embedded": False}, "library/appsec/2026-01-01_test")
    ...
    async def test_index_persists_to_disk(self, data_dir):
        ...
        idx = ItemIndex(data_dir)
        await idx.load()
        await idx.add("abc123", {**SAMPLE_ITEM, "content_hash": "sha256:aaa", "word_count": 10, "added": "2026-01-01T00:00:00Z", "updated": "2026-01-01T00:00:00Z", "embedded": False}, "library/appsec/test")
        # Reload from disk
    ...
    async def test_find_by_hash(self, data_dir):
        ...
        idx = ItemIndex(data_dir)
        await idx.load()
        await idx.add("abc123", {**SAMPLE_ITEM, "content_hash": "sha256:deadbeef", "word_count": 10, "added": "2026-01-01T00:00:00Z", "updated": "2026-01-01T00:00:00Z", "embedded": False}, "library/appsec/test")
    ...
    async def test_search_by_category(self, data_dir):
        ...
        await idx.add("id1", {**meta, "category": "appsec"}, "library/appsec/test1")
        await idx.add("id2", {**meta, "category": "netsec", "content_hash": "sha256:bbb"}, "library/netsec/test2")
    ...
    async def test_search_by_tag(self, data_dir):
        ...
        await idx.load()
        meta = {**SAMPLE_ITEM, "content_hash": "sha256:aaa", "word_count": 10, "added": "2026-01-01T00:00:00Z", "updated": "2026-01-01T00:00:00Z", "embedded": False}
        await idx.add("id1", {**meta, "tags": ["xss", "csrf"]}, "library/appsec/test1")
        await idx.add("id2", {**meta, "tags": ["mitm"], "content_hash": "sha256:bbb"}, "library/netsec/test2")
    ...
    async def test_search_text_query(self, data_dir):
        ...
        await idx.load()
        meta = {**SAMPLE_ITEM, "content_hash": "sha256:aaa", "word_count": 10, "added": "2026-01-01T00:00:00Z", "updated": "2026-01-01T00:00:00Z", "embedded": False}
        await idx.add("id1", {**meta, "title": "Kerberoasting guide", "summary": "AD attacks"}, "library/redteam/test1")
        await idx.add("id2", {**meta, "title": "Web fuzzing", "summary": "Fuzzing web apps", "content_hash": "sha256:bbb"}, "library/fuzzing/test2")
    ...
    async def test_rebuild_from_filesystem(self, data_dir):
        ...
        idx = ItemIndex(data_dir)
        await idx.load()
        await save_item(SAMPLE_ITEM, SAMPLE_CONTENT, idx, data_dir)
...
class TestFilesystem:
    ...
    async def test_save_and_load(self, data_dir):
        ...
        idx = ItemIndex(data_dir)
        await idx.load()
        item_id = await save_item(SAMPLE_ITEM, SAMPLE_CONTENT, idx, data_dir)
    ...
    async def test_dedup_by_hash(self, data_dir):
        ...
        idx = ItemIndex(data_dir)
        await idx.load()
        id1 = await save_item(SAMPLE_ITEM, SAMPLE_CONTENT, idx, data_dir)
